...
const myFormat = format.printf(log => {
  let formato;
  if (log.stack) {
    path = log.stack.split('\n')[1].slice(7); // debe quedar algo asi: (C:\\Users\\Ricardo\\Desktop\\Node\\thothify-api\\models\\User.js:45:15)
    buff = Buffer.from(path);
    let fileAndLineNumber = path.slice(buff.lastIndexOf('\\') + 1); // queda algo asi User.js:45:15)
